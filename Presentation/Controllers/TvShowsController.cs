...
namespace Presentation.Controllers
{
    ...
    public class TvShowsController : ControllerBase
    {
        ...
        [HttpGet("featured")]
        public async Task<ActionResult<ApiResponse<List<TvShowDetailsDto>>>> GetFeatured([FromQuery] int limit = 10)
        {
            try
            {
                var shows = await _tvShowService.GetFeaturedAsync(limit);
                if (shows == null || !shows.Any())
                    return NotFound(ApiResponse<List<TvShowDetailsDto>>.FailResponse("No featured shows found."));

                return Ok(ApiResponse<List<TvShowDetailsDto>>.SuccessResponse(shows.ToList(), "Featured shows retrieved."));
            }
            catch (Exception ex)
            {
                return BadRequest(ApiResponse<List<TvShowDetailsDto>>.FailResponse("Failed to retrieve featured shows", new List<string> { ex.Message }));
            }
        }

        [HttpGet]
        public async Task<ActionResult<ApiResponse<List<TvShowDetailsDto>>>> GetAllShows()
        {
            try
            {
